...
def exibir_sucesso_pendente():
    if st.session_state.get("msg_sucesso_pendente"):
        st.success(f"✅ {st.session_state.msg_sucesso_pendente}")
...
def is_grupo_concluido(banca):
    notas = banca.get("notas_lancadas", {})
    if banca.get("avaliador_1_email") and notas.get("Avaliador 1") is None: return False
    if banca.get("avaliador_2_email") and notas.get("Avaliador 2") is None: return False
    if banca['modulo'] not in ["MCM V", "PIEPE"]:
        ori = notas.get("Orientador")
        if not ori: return False
...
def gerar_texto_ata_defesa(banca):
    ...
    data_str = banca.get('data', 'Data não definida')
    
    texto = f"ATA DE DEFESA DE TRABALHO DE CONCLUSÃO DE CURSO - {banca['modulo']}\n"
    texto += "=" * 70 + "\n\n"
    
    texto += f"No dia {data_str}, reuniu-se a banca examinadora para a avaliação do Trabalho de Conclusão de Curso da graduação em Medicina, "
    ...
    texto += f"- Orientador(a): {banca.get('orientador_nome', 'N/A')}\n"
    if banca.get('avaliador_1_nome'): texto += f"- Prof. Titular 1: {banca.get('avaliador_1_nome')}\n"
    if banca.get('avaliador_2_nome'): texto += f"- Prof. Titular 2: {banca.get('avaliador_2_nome')}\n"
    if banca.get('avaliador_sup_nome'): texto += f"- Prof. Suplente: {banca.get('avaliador_sup_nome')}\n\n"
    
    texto += "ASSINATURAS ELETRÔNICAS REGISTRADAS NO SISTEMA:\n"
    assinaturas = banca.get("assinaturas_defesa", [])
    if not assinaturas:
        texto += "(Nenhuma assinatura registrada até o momento)\n"
    else:
        for ass in assinaturas:
            texto += f"[X] Assinado eletronicamente por: {ass}\n"
            
    texto += "\n" + "=" * 70 + "\n"
    texto += f"Documento gerado automaticamente pelo Portal CRIVO em {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
    return texto
    ...
    st.session_state.bancos_avaliacoes = [
        {
            ...
            "atas_mensais": {"Ata Mês 1": True, "Ata Mês 2": True, "Ata Mês 3": True, "Ata Mês 4": False}, "conteudo_atas": {},
            ...
            "ata_assinada": False, "assinaturas_defesa": [], "avaliadores_concluidos": [], "notas_detalhadas": {}, "migrado": False
        }
    ]
...
for b in st.session_state.bancos_avaliacoes:
    ...
    if "migrado" not in b: b["migrado"] = False
    novas_atas = {}
...
def tela_coordenacao():
    ...
    if aba_ativa == "➕ Cadastrar Grupos":
                ...
                with st.form(key=f"form_cadastro_v_{st.session_state.versao_formulario}", clear_on_submit=False):
                    ...
                    if st.form_submit_button("Salvar Registro Oficial"):
                        if not o_nome or not o_email or not lista_alunos: st.error("Orientador e Alunos são obrigatórios.")
                            ...
                            # TRAVA DE CHOQUE DE HORÁRIO NO CADASTRO
                            teve_conflito, conflitos = verificar_conflito_horario(formatar_data_br(data_banca) if data_banca else "A definir", hora_str, [o_email, co_email, b1_email, b2_email, bs_email])
                            if teve_conflito and hora_str != "N/A" and data_banca:
                                st.error(f"🚨 ALERTA DE CHOQUE! Os e-mails {conflitos} já estão ocupados em outra banca neste mesmo dia e horário.")
                            else:
        ...
        st.write("### Controle e Edição de Grupos")
        ...
        with st.expander("🗓️ Configurar Janelas (Agendamento e Notas por Turma)", expanded=False):
            ...
            with col_a1: dt_ag_ini = st.date_input("Início Agendamento:", value=st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo].get("agend_ini", datetime.now().date()), format="DD/MM/YYYY", key=f"d_ag_i_{mod_alvo}")
            with col_a2: dt_ag_fim = st.date_input("Fim Agendamento:", value=st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo].get("agend_fim", datetime.now().date()), format="DD/MM/YYYY", key=f"d_ag_f_{mod_alvo}")
            st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo]["agend_ini"] = dt_ag_ini; st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo]["agend_fim"] = dt_ag_fim
                
            st.markdown(f"**2. Restrições Logísticas ({mod_alvo}):**")
            col_s1, col_s2 = st.columns(2)
            with col_s1: salas_sel = st.multiselect("Salas APG:", lista_salas_base, default=st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo].get("salas", lista_salas_base), key=f"salas_mult_{mod_alvo}")
            with col_s2: horas_sel = st.multiselect("Horários:", [t.strftime('%H:%M') for t in lista_horarios_base], default=st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo].get("horarios", [t.strftime('%H:%M') for t in lista_horarios_base]), key=f"horas_mult_{mod_alvo}")
            st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo]["salas"] = salas_sel; st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo]["horarios"] = horas_sel
                
            st.markdown(f"**3. Período de LANÇAMENTO DE NOTAS ({mod_alvo}):**")
            col_n1, col_n2 = st.columns(2)
            with col_n1: dt_n_ini = st.date_input("Início das Notas:", value=st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo].get("notas_ini", datetime.now().date()), format="DD/MM/YYYY", key=f"d_n_i_{mod_alvo}")
            with col_n2: dt_n_fim = st.date_input("Fim das Notas:", value=st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo].get("notas_fim", datetime.now().date()), format="DD/MM/YYYY", key=f"d_n_f_{mod_alvo}")
            st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo]["notas_ini"] = dt_n_ini; st.session_state.configuracoes["disponibilidade_por_modulo"][mod_alvo]["notas_fim"] = dt_n_fim
        ...
        # MIGRAÇÃO EM LOTE
        if filtro_gestao_mod in ["TCC I", "MCM IV"] and bancas_filtradas_gestao:
            novo_mod_lote = "TCC II" if filtro_gestao_mod == "TCC I" else "MCM V"
            if st.button(f"🚀 Migrar Todos os Grupos Concluídos ({filtro_gestao_mod} ➡️ {novo_mod_lote})", use_container_width=True):
                migrados = 0
                for b in bancas_filtradas_gestao:
                    if not b.get("migrado", False) and is_grupo_concluido(b):
                        curr_sem = b.get('semestre', '2026.1')
                        try:
                        ...
                        b_copy = b.copy()
                        b_copy.update({"id": str(uuid.uuid4())[:8], "modulo": novo_mod_lote, "semestre": novo_sem, "status": "Em Orientação", "avaliadores_concluidos": [], "notas_detalhadas": {}, "assinaturas_defesa": [], "migrado": False, "notas_lancadas": {"Orientador": {}, "Avaliador 1": None, "Avaliador 2": None, "Suplente": None}})
                        st.session_state.bancos_avaliacoes.append(b_copy)
                        
                        indice_real = next((i for i, d in enumerate(st.session_state.bancos_avaliacoes) if d["id"] == b["id"]), None)
                        st.session_state.bancos_avaliacoes[indice_real]["migrado"] = True
                        migrados += 1
                if migrados > 0: recarregar_com_sucesso(f"Sucesso! {migrados} grupos foram migrados para {novo_mod_lote}.")
                else: st.warning("Nenhum grupo apto (com todas as notas lançadas) para migração.")
                
        # EXPORTAÇÃO AUTO-AJUSTÁVEL DE GESTÃO
        with st.expander("📊 Exportar Planilha Atual de Gestão"):
            ...
            if not df_export.empty:
                ...
                df_export = df_export.drop(columns=['alunos', 'atas_mensais', 'notas_lancadas', 'ata_assinada', 'conteudo_atas', 'assinaturas_defesa', 'avaliadores_concluidos', 'notas_detalhadas'], errors='ignore')
                ...
                st.download_button("📥 Baixar Planilha de Gestão (.xlsx)", data=buffer_g.getvalue(), file_name=f"Bancas_Gestao.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        ...
        if not bancas_filtradas_gestao: 
            ...
            for banca in reversed(bancas_filtradas_gestao):
                ...
                if banca['modulo'] == "PIEPE": info_extras += f" | Formato: {banca.get('formato_piepe', 'Slide')}"
                ...
                with st.container(border=True):
                    ...
                    with col_b3:
                        ...
                        if novo_mod != banca['modulo'] and not banca.get("migrado"):
                            if is_grupo_concluido(banca):
                                if st.button(f"🔄 Migrar p/ {novo_mod}", key=f"mig_{banca['id']}", use_container_width=True):
                                    ...
                                    b_copy = banca.copy()
                                    b_copy.update({"id": str(uuid.uuid4())[:8], "modulo": novo_mod, "semestre": novo_sem, "status": "Em Orientação", "avaliadores_concluidos": [], "notas_detalhadas": {}, "assinaturas_defesa": [], "migrado": False, "notas_lancadas": {"Orientador": {}, "Avaliador 1": None, "Avaliador 2": None, "Suplente": None}})
                                    st.session_state.bancos_avaliacoes.append(b_copy)
                                    st.session_state.bancos_avaliacoes[indice_real]["migrado"] = True
                                    recarregar_com_sucesso(f"Migrado para {novo_mod} ({novo_sem})!")
                            else:
                                st.button("🔒 Notas Pendentes", key=f"mig_{banca['id']}", disabled=True, use_container_width=True, help="O grupo precisa de todas as notas preenchidas para migrar.")
                    ...
                    if st.session_state[edit_key]:
                        ...
                        with st.form(key=f"form_ed_real_{banca['id']}", clear_on_submit=False):
                            ...
                            if salvar_alt:
                                ...
                                # TRAVA DE CHOQUE DE HORÁRIO NA EDIÇÃO
                                teve_conflito, conflitos = verificar_conflito_horario(formatar_data_br(edit_data), h_str, [e_oe, e_coe, e_b1e, e_b2e, e_bse], id_ignorar=banca['id'])
                                if teve_conflito and h_str != "N/A" and formatar_data_br(edit_data) != "A definir":
                                    st.error(f"🚨 ALERTA DE CHOQUE! Os e-mails {conflitos} já estão ocupados em outra banca neste mesmo dia e horário.")
                                else:
        ...
        if not bancas_monitoramento: 
            ...
            for banca in bancas_monitoramento:
                ...
                with st.container(border=True):
                    ...
                    st.markdown(f"**Banca:** {av_str}", unsafe_allow_html=True)
                    ...
                    if banca['modulo'] != "PIEPE":
                        ...
                        with col_a1: st.markdown(f"Ata Mês 1: {'✅' if atas.get('Ata Mês 1') else '❌'}"); st.download_button("📥 Baixar Ata (.txt)", txt_m1, key=f"dm1_{banca['id']}", file_name=f"Ata_M1_{banca['id']}.txt", use_container_width=True)
                        with col_a2: st.markdown(f"Ata Mês 2: {'✅' if atas.get('Ata Mês 2') else '❌'}"); st.download_button("📥 Baixar Ata (.txt)", txt_m2, key=f"dm2_{banca['id']}", file_name=f"Ata_M2_{banca['id']}.txt", use_container_width=True)
                        with col_a3: st.markdown(f"Ata Mês 3: {'✅' if atas.get('Ata Mês 3') else '❌'}"); st.download_button("📥 Baixar Ata (.txt)", txt_m3, key=f"dm3_{banca['id']}", file_name=f"Ata_M3_{banca['id']}.txt", use_container_width=True)
                        with col_a4: st.markdown(f"Ata Mês 4: {'✅' if atas.get('Ata Mês 4') else '❌'}"); st.download_button("📥 Baixar Ata (.txt)", txt_m4, key=f"dm4_{banca['id']}", file_name=f"Ata_M4_{banca['id']}.txt", use_container_width=True)
                    ...
                    if banca['modulo'] in ["TCC I", "TCC II", "MCM IV", "MCM V"]:
                        ...
                        st.write(f"**Assinaturas da Ata de Defesa:** {len(assinaturas)} de {qtd_necessaria} assinadas eletronicamente.")
                        texto_ata = gerar_texto_ata_defesa(banca)
                        st.download_button("📥 Baixar Ata de Defesa (.txt)", texto_ata, file_name=f"Ata_Defesa_{banca['id']}.txt", key=f"d_def_{banca['id']}")
        ...
        st.write("### Planilha Auto-ajustável para Excel")
...
def tela_professor():
    ...
    if aba_prof == "🎓 Meus Grupos (Orientação)":
        st.info("Aqui você gerencia Atas Mensais e a sua Nota de Orientação. Os alunos já avaliados são ocultados da área de notas.")
        ...
        for banca in orientacoes:
            ...
            with st.container(border=True):
                ...
                if banca['modulo'] != "PIEPE":
                    with st.expander("📅 Ver Controle e Formulários de Atas Mensais", expanded=False):
                ...
                if banca['modulo'] not in ["MCM V", "PIEPE"]:
                    st.markdown("---")
                    max_nota_txt = "40" if banca['modulo'] in ["TCC I", "TCC II"] else "70"
                    st.markdown(f"#### 📝 Rubrica de Avaliação do Orientador (Máximo {max_nota_txt} pontos)")
                    ...
                    if not pendentes:
                        st.success("✅ Todos os alunos deste grupo já foram avaliados pelo Orientador. Você pode consultar as notas na aba 'Histórico'.")
                    else:
                        mesma_nota = st.checkbox("Atribuir a mesma nota para TODOS os alunos pendentes de uma vez", value=False, key=f"msm_{banca['id']}")
                        if mesma_nota:
                            st.write("*Avaliação Geral (Aplica-se aos pendentes)*")
                            with st.form(key=f"form_g_{banca['id']}"):
                                if banca['modulo'] in ["TCC I", "TCC II"]:
                                    ...
                                    o2 = st.slider("Relação com Orientador/Diálogo (0 a 10):", 0.0, 10.0, 0.0, step=0.5, key=f"o2_g_{banca['id']}", help="Relação colaborativa, abertura ao diálogo e aceitação de sugestões.")
                                    ...
                                    o1 = st.slider("Envolvimento e Responsabilidade (0 a 14):", 0.0, 14.0, 0.0, step=0.5, key=f"o1_g_{banca['id']}", help="Participação proativa e responsabilidade no processo.")
                                    o2 = st.slider("Relação com Orientador/Diálogo (0 a 14):", 0.0, 14.0, 0.0, step=0.5, key=f"o2_g_{banca['id']}", help="Relação colaborativa e abertura.")
                                    o3 = st.slider("Desempenho e Tarefas (0 a 14):", 0.0, 14.0, 0.0, step=0.5, key=f"o3_g_{banca['id']}", help="Competência nas atividades.")
                                    o4 = st.slider("Pontualidade e Compromisso (0 a 14):", 0.0, 14.0, 0.0, step=0.5, key=f"o4_g_{banca['id']}", help="Compromisso com prazos.")
                                    o5 = st.slider("Resp. com Aprendizagem (0 a 14):", 0.0, 14.0, 0.0, step=0.5, key=f"o5_g_{banca['id']}", help="Busca ativa por aprendizado.")
                                
                                assinar_defesa = False
                                if banca['modulo'] in ["TCC II", "MCM V"]: assinar_defesa = st.checkbox("✍️ Assinar Ata de Defesa Eletronicamente agora", key=f"ass_g_{banca['id']}")
                                
                                if st.form_submit_button("Salvar Nota para Todos Pendentes"):
                                    nota_ori = o1 + o2 + o3 + o4 + o5
                                    notas_dit = {"Envolvimento": o1, "Diálogo": o2, "Tarefas": o3, "Pontualidade": o4, "Aprendizagem": o5, "Total do Orientador": nota_ori}
                                    ...
                                    hist_atual = st.session_state.bancos_avaliacoes[indice_real]['notas_detalhadas'].get(meu_email, {})
                                    hist_atual.update(notas_dit)
                                    st.session_state.bancos_avaliacoes[indice_real]['notas_detalhadas'][meu_email] = hist_atual
                                    if meu_email not in st.session_state.bancos_avaliacoes[indice_real]['avaliadores_concluidos']:
                                        st.session_state.bancos_avaliacoes[indice_real]['avaliadores_concluidos'].append(meu_email)
                                    if assinar_defesa and meu_email not in st.session_state.bancos_avaliacoes[indice_real]['assinaturas_defesa']:
                                        st.session_state.bancos_avaliacoes[indice_real]['assinaturas_defesa'].append(meu_email)
                                    recarregar_com_sucesso("Notas aplicadas com sucesso! Alunos foram avaliados.")
                        else:
                            st.write("*Avaliação Individual (Apenas Pendentes)*")
                            aluno_alvo = st.selectbox("Selecione o aluno para avaliar:", pendentes, key=f"sel_aluno_{banca['id']}")
                            idx_aluno = pendentes.index(aluno_alvo)
                            safe_aluno = "".join([c for c in aluno_alvo if c.isalnum()])
                            
                            with st.form(key=f"form_ind_{safe_aluno}_{banca['id']}"):
                                st.markdown(f"<b>👤 Avaliando: {aluno_alvo}</b>", unsafe_allow_html=True)
                                if banca['modulo'] in ["TCC I", "TCC II"]:
                                    o1 = st.slider("Envolvimento (0-10):", 0.0, 10.0, 0.0, step=0.5, key=f"o1_{safe_aluno}_{banca['id']}", help="Participação e responsabilidade.")
                                    o2 = st.slider("Diálogo (0-10):", 0.0, 10.0, 0.0, step=0.5, key=f"o2_{safe_aluno}_{banca['id']}", help="Abertura ao diálogo e sugestões.")
                                    o3 = st.slider("Tarefas (0-8):", 0.0, 8.0, 0.0, step=0.5, key=f"o3_{safe_aluno}_{banca['id']}", help="Desempenho nas atividades.")
                                    o4 = st.slider("Pontualidade (0-6):", 0.0, 6.0, 0.0, step=0.5, key=f"o4_{safe_aluno}_{banca['id']}", help="Compromisso com prazos.")
                                    o5 = st.slider("Aprendizagem (0-6):", 0.0, 6.0, 0.0, step=0.5, key=f"o5_{safe_aluno}_{banca['id']}", help="Busca por aprimoramento.")
                                else:
                                    o1 = st.slider("Envolvimento (0-14):", 0.0, 14.0, 0.0, step=0.5, key=f"o1_{safe_aluno}_{banca['id']}", help="Participação e responsabilidade.")
                                    o2 = st.slider("Diálogo (0-14):", 0.0, 14.0, 0.0, step=0.5, key=f"o2_{safe_aluno}_{banca['id']}", help="Abertura ao diálogo e sugestões.")
                                    o3 = st.slider("Tarefas (0-14):", 0.0, 14.0, 0.0, step=0.5, key=f"o3_{safe_aluno}_{banca['id']}", help="Desempenho nas atividades.")
                                    o4 = st.slider("Pontualidade (0-14):", 0.0, 14.0, 0.0, step=0.5, key=f"o4_{safe_aluno}_{banca['id']}", help="Compromisso com prazos.")
                                    o5 = st.slider("Aprendizagem (0-14):", 0.0, 14.0, 0.0, step=0.5, key=f"o5_{safe_aluno}_{banca['id']}", help="Busca por aprimoramento.")
                                
                                assinar_defesa = False
                                if banca['modulo'] == "TCC II" and len(pendentes) == 1: 
                                    assinar_defesa = st.checkbox("✍️ Assinar Ata de Defesa Eletronicamente (Último aluno)", key=f"ass_ind_{safe_aluno}_{banca['id']}")
                                
                                if st.form_submit_button(f"Salvar Nota de {aluno_alvo}"):
                                    nota_ind = o1 + o2 + o3 + o4 + o5
                                    notas_dit = {f"{aluno_alvo} - Envolvimento": o1, f"{aluno_alvo} - Diálogo": o2, f"{aluno_alvo} - Tarefas": o3, f"{aluno_alvo} - Pontualidade": o4, f"{aluno_alvo} - Aprendizagem": o5, f"Total ({aluno_alvo})": nota_ind}
                                    notas_existentes[aluno_alvo] = nota_ind
                                    st.session_state.bancos_avaliacoes[indice_real]['notas_lancadas']['Orientador'] = notas_existentes
                                    hist_atual = st.session_state.bancos_avaliacoes[indice_real]['notas_detalhadas'].get(meu_email, {})
                                    hist_atual.update(notas_dit)
                                    st.session_state.bancos_avaliacoes[indice_real]['notas_detalhadas'][meu_email] = hist_atual
                                    ...
                                    if assinar_defesa and meu_email not in st.session_state.bancos_avaliacoes[indice_real]['assinaturas_defesa']:
                                        st.session_state.bancos_avaliacoes[indice_real]['assinaturas_defesa'].append(meu_email)
                                    recarregar_com_sucesso(f"Nota de {aluno_alvo} salva com sucesso!")

                if banca['modulo'] == "MCM V" and meu_email not in banca.get("assinaturas_defesa", []):
                    st.markdown("---")
        ...
        if not bancas_pendentes: st.success("🎉 Você não tem bancas pendentes de avaliação no momento.")
            ...
            for banca in bancas_pendentes:
                ...
                with st.container(border=True):
                    max_nota_txt = "100" if banca['modulo'] in ["MCM V", "PIEPE"] else ("30" if banca['modulo'] == "MCM IV" else "60")
                    st.markdown(f"#### {banca['modulo']} - {banca.get('titulo', 'Sem Título')}")
                    st.markdown(f"**Avaliação Máxima: {max_nota_txt} pontos.** | **Alunos:** {', '.join(banca.get('alunos', []))}")
                    ...
                    if not janela_notas:
                        ...
                        with st.container(height=450, border=True):
                            ...
                            elif banca['modulo'] == "MCM V":
                                st.markdown("##### 📝 Parte Escrita - Artigo/Projeto (Máx 50 pontos)")
                                c1 = st.slider("1. Estruturação e Escrita Científica (0 a 12.5):", 0.0, 12.5, 0.0, step=0.5, key=f"m5_c1_{banca['id']}", help="Estrutura organizada e linguagem científica apropriada.")
                                c2 = st.slider("2. Fundamentação Teórica e Qualidade das Referências (0 a 10.0):", 0.0, 10.0, 0.0, step=0.5, key=f"m5_c2_{banca['id']}", help="Crítica, atualizada e com base em diretrizes oficiais (SBC, SBPT, etc).")
                                c3 = st.slider("3. Apresentação e Discussão dos Resultados (0 a 10.0):", 0.0, 10.0, 0.0, step=0.5, key=f"m5_c3_{banca['id']}", help="Resultados claros e bem discutidos.")
                                c4 = st.slider("4. Metodologia (0 a 10.0):", 0.0, 10.0, 0.0, step=0.5, key=f"m5_c4_{banca['id']}", help="Métodos bem descritos e executáveis.")
                                c5 = st.slider("5. Conclusão e Relevância (0 a 7.5):", 0.0, 7.5, 0.0, step=0.5, key=f"m5_c5_{banca['id']}", help="Conclusão clara e alinhada aos objetivos.")
                                n_escrita = c1+c2+c3+c4+c5
                                
                                st.markdown("##### 🗣️ Apresentação Oral (Máx 50 pontos)")
                                o1 = st.slider("Domínio do Tema e Clareza (0 a 17.0):", 0.0, 17.0, 0.0, step=0.5, key=f"m5_o1_{banca['id']}", help="Segurança na exposição do raciocínio e conteúdo.")
                                o2 = st.slider("Capacidade de Síntese e Tempo (15-20 minutos) (0 a 16.5):", 0.0, 16.5, 0.0, step=0.5, key=f"m5_o2_{banca['id']}", help="Uso adequado do tempo e recursos visuais.")
                                o3 = st.slider("Arguição e Defesa (0 a 16.5):", 0.0, 16.5, 0.0, step=0.5, key=f"m5_o3_{banca['id']}", help="Segurança, clareza e capacidade de defendesa perante a banca.")
                                n_oral = o1+o2+o3
                                
                                nota_final_calc = n_escrita + n_oral
                                notas_dit = {"Escrita/Estrutura": c1, "Fundamentação": c2, "Terapêutica/Res.": c3, "Metodologia/Raciocínio": c4, "Conclusão": c5, "Domínio Oral": o1, "Síntese/Tempo": o2, "Arguição": o3, "Total da Banca": nota_final_calc}
                                if any(v == 0 for v in [c1, c2, c3, c4, c5, o1, o2, o3]): zeros = True
                            ...
                            assinar_defesa = False
                            if banca['modulo'] in ["TCC II", "MCM V"]:
                                assinar_defesa = st.checkbox("✍️ Assinar Ata de Defesa Eletronicamente agora", key=f"ass_{banca['id']}")
        ...
        with col_h1: f_hist_mod = st.selectbox("Filtrar por Módulo:", ["Todos", "TCC I", "TCC II", "MCM IV", "MCM V", "PIEPE"], key="fh_mod")
        with col_h2: f_hist_sem = st.selectbox("Filtrar por Semestre:", ["Todos"] + lista_semestres, key="fh_sem")
        st.markdown("---")
        
        bancas_avaliadas = [b for b in st.session_state.bancos_avaliacoes if meu_email in b.get('avaliadores_concluidos', [])]
        bancas_avaliadas = [b for b in bancas_avaliadas if (f_hist_mod == "Todos" or b['modulo'] == f_hist_mod) and (f_hist_sem == "Todos" or b.get('semestre') == f_hist_sem)]
        ...
        for banca in bancas_avaliadas:
            indice_real = next((i for i, d in enumerate(st.session_state.bancos_avaliacoes) if d["id"] == banca["id"]), None)
            with st.container(border=True):
                st.write(f"✅ **{banca['modulo']} ({banca.get('semestre')})** - {banca.get('titulo')} | Alunos: {', '.join(banca['alunos'])}")
                ...
                if banca['modulo'] in ["TCC II", "MCM V"] and meu_email not in banca.get("assinaturas_defesa", []):
                    st.markdown("---")
                    if st.button("✍️ Assinar Ata de Defesa Eletronicamente", key=f"assina_banca_{banca['id']}", use_container_width=True):
                        st.session_state.bancos_avaliacoes[indice_real]['assinaturas_defesa'].append(meu_email)
                        recarregar_com_sucesso("Assinatura eletrônica computada na Ata de Defesa!")
