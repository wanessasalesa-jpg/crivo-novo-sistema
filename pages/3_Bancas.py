...
# ==========================================
# MÓDULO DE LOGIN (Liberado para externos cadastrados)
# ==========================================
def tela_login():
        ...
        with st.form("form_login"):
            ...
            if submit:
                if email not in st.session_state.permissoes_acesso:
                    st.error("E-mail não cadastrado. Procure a coordenação do módulo.")
                elif senha != "afya2026": 
...
# ==========================================
# PAINEL 0: ADMINISTRAÇÃO (Master com Edição/Exclusão)
# ==========================================
def tela_administracao():
    ...
    st.info("Delegue ou revogue os acessos dos Coordenadores. Nenhuma banca é perdida se um coordenador for removido.")
        ...
        with st.form("form_add_coord", clear_on_submit=True):
            ...
            if st.form_submit_button("Conceder Acesso de Coordenação"):
                if not novo_email_coord.endswith("@afya.com.br"):
                    st.error("Insira um e-mail válido da Afya para a coordenação.")
    ...
    st.markdown("#### Gestão de Coordenadores Ativos")
    # Filtra apenas quem é coordenador para não mostrar os avaliadores
    coordenadores = {e: d for e, d in st.session_state.permissoes_acesso.items() if isinstance(d, dict) and d.get("perfil") == "Coordenação"}
    
    if not coordenadores:
        st.write("Nenhum coordenador cadastrado.")
    
    for email, dados in coordenadores.items():
        with st.container(border=True):
            st.markdown(f"**👤 {formatar_nome_email(email)}** ({email})")
            
            # Painel de Edição de Módulos
            ...
            with col2:
                if st.button("🗑️ Revogar Acesso", key=f"del_{email}", use_container_width=True):
    ...
    with aba_criar:
        ...
        if not modulos_permitidos:
            ...
            if modulo_selecionado == "PIEPE":
                st.warning("⚠️ PIEPE: Defina o formato da apresentação. Horário não se aplica.")
            ...
            if modulo_selecionado:
                with st.form("form_nova_banca", clear_on_submit=True):
                    ...
                    with col_hr:
                        if modulo_selecionado != "PIEPE":
                            horario_banca = st.time_input("Horário de Início:")
                        else:
                            horario_banca = None
                    ...
                    st.markdown("---")
                    st.write("**Orientação** *(Permitido: @afya.com.br ou @parceiro.afya.com.br)*")
                    col_o1, col_o2 = st.columns(2)
                    with col_o1:
                        orientador_email = st.text_input("E-mail do Orientador:").lower().strip()
                    ...
                    if modulo_selecionado in ["TCC I", "MCM IV"]:
                        ...
                        with col_b1:
                            avaliador_1_email = st.text_input("E-mail Titular 1:").lower().strip()
                        with col_b2:
                            avaliador_2_email = st.text_input("E-mail Titular 2:").lower().strip()
                        with col_b3:
                            avaliador_sup_email = st.text_input("E-mail Suplente:").lower().strip()
                        
                    st.markdown("---")
                    st.write("**Integrantes do Grupo**")
                    lista_alunos = st.text_area("Nomes dos Alunos (Cole a lista com um por linha):", height=150)
                    ...
                    if btn_salvar:
                        ...
                        if not titulo or not orientador_email or not avaliador_1_email or not avaliador_2_email or not lista_alunos:
                            ...
                            # Liberando acessos ignorando domínios para os avaliadores
    ...
    with aba_gerenciar:
        if not st.session_state.bancos_avaliacoes:
            ...
            with st.expander("📊 Relatório Consolidado (Exportar Excel)"):
                ...
                # GERADOR DE EXCEL INTELIGENTE COM AUTO-AJUSTE
                buffer = io.BytesIO()
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    ...
                    for idx, col in enumerate(df_export.columns):
                        ...
                        col_letter = chr(65 + idx) # Funciona perfeitamente até a coluna Z
            ...
            for i, banca in enumerate(reversed(st.session_state.bancos_avaliacoes)):
                ...
                info_extras = f"{banca['data']}"
                if banca.get('horario') and banca['horario'] != "N/A": info_extras += f" às {banca['horario']}"
                ...
                with st.container(border=True):
                    ...
                    if st.session_state[edit_key]:
                        ...
                        with st.form(key=f"form_edit_{banca['id']}"):
                            ...
                            try:
                                h, m = map(int, banca.get('horario', '00:00').split(':'))
                                hora_obj = time(h, m)
                            except:
                                hora_obj = time(8, 0)
                            ...
                            with col_ed2:
                                if banca['modulo'] != "PIEPE":
                                    edit_hora = st.time_input("Horário:", value=hora_obj)
                                else:
                                    edit_hora = None
...
# ==========================================
# PAINEL 2: AVALIADOR (Com Trava Temporal)
# ==========================================
def tela_avaliador():
    col_titulo, col_logout = st.columns([4, 1])
    with col_titulo:
    ...
    st.info("Aqui ficam as bancas para as quais você foi convidado. ⚠️ A avaliação só será liberada no dia e horário exatos agendados pela coordenação.")
